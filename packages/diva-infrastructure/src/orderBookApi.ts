...
export function orderBookApi(construct: Construct) {
  ...
  const lambdaValidatorRole = new IamRole(
    construct,
    id("OrderbookLambdaValidatorRole"),
    {
      name: id("OrderbookLambdaValidatorRole"),
      assumeRolePolicy: JSON.stringify({
        Version: "2012-10-17",
        Statement: [
          {
  ...
  const validateOrderFileName = `${process.cwd()}/dist/lambda-fns/validateOrders.zip`;

  const lambdaValidator = new LambdaFunction(
    construct,
    id("OrderbookValidator"),
    {
      filename: validateOrderFileName,
      runtime: "nodejs12.x",
      handler: "index.handler",
      role: lambdaAuthorizerRole.arn,
      sourceCodeHash: fileHash(authFileName),
      functionName: id("OrderbookValidator"),
      tags: tags({}),
    }
  );

  const validatorRule = new CloudwatchEventRule(construct, id('OrderbookValidatorCron'), {
    name: id('OrderbookValidatorCron'),
    description: 'Validator for Orderbook Lambda',
    scheduleExpression: 'rate(5 minutes)'
  })

  new CloudwatchEventTarget(construct, id('OrderbookValidatorEventTarget'), {
    arn: lambdaValidator.arn,
    rule: validatorRule.name,
    targetId: id('OrderbookValidatorEventTarget')
  })
  ...
  new CloudwatchLogGroup(construct, id("OrderbookValidatorLogs"), {
    name: `/aws/lambda/${lambdaValidator.functionName}`,
    retentionInDays: 14,
    tags: tags({}),
  });
  ...
  const lambdaDd = new IamPolicy(construct, id("LambdaDynamoDbAccess"), {
    name: id("LambdaDynamoDbAccess"),
    path: "/",
    description: "IAM policy for access to dynamodb from lambda",
    policy: JSON.stringify({
      Version: "2012-10-17",
      Statement: [
        {
          Action: [
            // TODO - make permissions narrower
            "dynamodb:*",
          ],
  ...
  new IamRolePolicyAttachment(construct, id("LambdaValidatorDdAttachment"), {
    role: lambdaValidatorRole.name,
    policyArn: lambdaDd.arn,
  });

  new IamRolePolicyAttachment(construct, id("LambdaValidatorLogAttachment"), {
    role: lambdaValidatorRole.name,
